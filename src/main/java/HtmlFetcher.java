...
public class HtmlFetcher {
			...
			if (header != null) {
				if (header.equalsIgnoreCase("Content-Type")) {
					if (!headers.get(header).isEmpty()) {
						if (headers.get(header).get(0).contains("text/html")) {
							return true;
						}
					}
				}
	...
	public static int getStatusCode(Map<String, List<String>> headers) {
		...
		for (String header : headers.keySet()) {
			if (header == null) {
				if (!headers.get(header).isEmpty()) {
					try {
						Matcher matcher = pattern.matcher(headers.get(header).get(0));
						if (matcher.matches()) {
							return Integer.valueOf(matcher.group(1));
						}
					} catch (Exception e) {
						System.out.println("Error while getting status code");
					}
				}
			}
		}
	...
	public static String fetch(URL url, int redirects) {
		try {
			...
			if (isRedirect(results) && redirects > 0) {
				URL newRedirect = new URL(String.join("", results.get("Location")));
				return fetch(newRedirect, redirects - 1);
			}

		} catch (Exception e) {
			System.out.println("Error fetching URL: "+url.toString());
		}
		return null;
	}
